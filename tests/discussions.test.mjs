...
(async () => {
    ...
    try {
        ...
        topicId = topicRes.body.message.name;
        ...
        try {
            ...
            if (topicId) {
                await client.query('DELETE FROM discussion_replies WHERE topic_id = $1', [topicId]);
                await client.query('DELETE FROM discussion_topics WHERE id = $1', [topicId]);
            }
            await client.query("DELETE FROM discussion_topics WHERE title LIKE 'Discussion Thread %' OR title = 'Hacked'");
            await client.end();
