...
(async () => {
    ...
    try {
        ...
        liveTitle = `Architecture AMA ${Date.now().toString(36)}`;
        ...
        try {
            ...
            if (liveTitle) {
                await client.query('DELETE FROM live_classes WHERE title = $1', [liveTitle]);
            }
            await client.query("DELETE FROM live_classes WHERE title LIKE 'Architecture AMA %' OR meet_link = 'https://meet.google.com/xyz-uvwx-rst'");
            await client.query("DELETE FROM reviews WHERE member_id IN (SELECT id FROM users WHERE email = $1 OR email LIKE 'reviewer-%@example.com')", [studentEmail]);
            await client.query("DELETE FROM users WHERE email = $1 OR email LIKE 'reviewer-%@example.com'", [studentEmail]);
            await client.end();
        } catch (_) { /* non-fatal teardown */ }
    }
    process.exit(0);
})().catch((err) => {
