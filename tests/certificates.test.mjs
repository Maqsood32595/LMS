...
(async () => {
    ...
    try {
        ...
        certId = issueRes.body.message.certificate_id;
        ...
        try {
            ...
            if (certId) {
                await client.query('DELETE FROM certificates WHERE certificate_id = $1', [certId]);
            }
            await client.query('DELETE FROM users WHERE email = $1', [studentEmail]);
            await client.query("DELETE FROM certificates WHERE member_id IN (SELECT id FROM users WHERE email LIKE 'grad-%@example.com')");
            await client.query("DELETE FROM users WHERE email LIKE 'grad-%@example.com'");
            await client.end();
